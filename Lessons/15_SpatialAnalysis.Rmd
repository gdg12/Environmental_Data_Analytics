...
EPAair_PM25_NC2018_raw <- read.csv("EPAair_PM25_NC2018_raw.csv")
...
ggplot(EPAair_PM_avg, aes(x=SITE_LONGITUDE, y=SITE_LATITUDE)) + 
  geom_point(aes(color=meanPM), size=4) + coord_equal() #scales x and y values 
```
...
#Set the available map backgrounds (set basemaps)
...
myMap = mapview(EPAair_PM_avg,   ###dataframe
         xcol = "SITE_LONGITUDE",  #xvalue-must be longitude
         ycol = "SITE_LATITUDE",    #yvalue-must be latitude
...
```{r}
myMap  <- leaflet(data=EPAair_PM_avg) %>%   ###dataframe
  addTiles() %>% ##tiles are the background maps-default tile is the open street map. GGMap uses Google Map as a reference map
  addCircleMarkers(~SITE_LONGITUDE,~SITE_LATITUDE,
                   radius=(~meanPM*2),  ##scaled mean particulate values
                   stroke = FALSE, 
                   fillOpacity = 0.3,
                   popup = ~as.character(`Site.Name`))  ###designate field to popup, and you can size and color it by attributes

###If you put map in viewer (not in console, click publish to publish map to the internet using Rpubs)
```
...
In our EPA example, we used a _coordinate pair (e.g. Latitude/Longitude) to represent the location of our point features. _But how might line and polygon features represented?
...
###Geometry will be a column, with (x,y) coordinates as observations. There will be two or more coordinate points for each observation
...
##st as sf converts data frame to sf, which is a spatial feature data frame
##also specify what columns define geometry
...
#Plot the geometry...specify geometry column
```{r}
plot(EPAair_PM_avg_sf$geometry)
```
##Shows points in gegraphic space
If you don't specify specific attributes, this will plot all of the attributes
...
```
##geom_sf menas plot goemetry as a spatial feature. This shows our values asd a geometry specifying the data. It extracts location information from the geometry column. 
##All of the points are colored by the attribute meanPM
...
Durham_buffer <- EPAair_PM_avg_sf %>% 
  ...
  st_buffer(0.1)  #how many units of buffering-0.1 degrees, since our data is in longitude and latitude
###Lat and long values are not distance units-they are angular units, and are points on a sphere

```
##Filter the original data frame only including Durham Armory value-one single row
...
```
###The st_crs specifies what coordinate reference system the geometry uses. 
###EPSG is a petroleum group=4326 refers to a specific coordinate system
###proj4string tells us the projection name "longlat", the datum name(WGS84), and no other definitions
...
#Transform the entire EPA dataset to the UTM Zone 17N crs (world is cut up into different zones)
###Take angular degrees and transform them into planar degres
###Converts existing spatial data frame to coordinate reference system
...
```{r Distance analysis} 
#Compute the distance between the Durham Armory site and all other sites in EPA dataset

###first we're filtering to the durham armory site
###then, st_distance supplies an x and a y, the output is a list of pairwise distances
Distances_to_DASite <- EPAair_PM_avg_sf_UTM %>% 
  ...
  data.frame() %>%  t                         #Converts list to a data frame and Transpose the result


```

###This list has 24 entries
...
```{r read the USA county shapefile, subsetting for NC}
install.packages(sf)
library(sf)
library(dplyr)
setwd("~/Desktop/Environmental Data Analytics/Environmental_Data_Analytics/Data/Spatial")

counties_sf<- st_read('cb_2017_us_county_20m.shp') %>% 
  filter(STATEFP == 37) #Filter for just NC Counties

###St_read reads in spatial data and puts it into a spatial dataframe
...
setwd("~/Desktop/Environmental Data Analytics/Environmental_Data_Analytics/Data/Spatial")
huc8_sf <-st_read('huc_250k_nc.shp')
...
###Change gratiucle 
plot(huc8_sf(SUB), graticule=st_crs(huc8_sf), axes=TRUE)
```
...
setwd("~/Desktop/Environmental Data Analytics/Environmental_Data_Analytics/Data/Spatial")
upperNeuse_sf <-st_read('huc_250k_nc.shp')%>%filter(HUC_NAME=="Upper Neuse")
...
#Aggregate the data using group_by and summarize, just as you would a non-spatial dataframe so that all of the counties are one feature
...
mapview(state_sf2)
```
###st_union is a geometric operation that unites all the results
...
### ggplot
```{r Visualizing mulitple dataset with ggplot}
#Wrong order
ggplot()  +
...
###Select HUCS and clip them by the state. Thec 
...
setwd("~/Desktop/Environmental Data Analytics/Environmental_Data_Analytics/Data/Spatial")
demog_df <- read.csv('NC_Demography.csv') %>% ##creating demography data frame
  mutate(FIPS = as.factor(FIPS))                                 #Convert the FIPS variable(a number) to a factor
```
...
```{r}
counties_sf_utm_join <- counties_sf_utm %>% ###counties spatial features 
  left_join(y = demog_df,by = c("GEOID" =  "FIPS"))  ###left table comes first
##GEOID is from county feature class
```
...
```{r}
dim(counties_sf_utm)
dim(demog_df)  ###100 rows, 15 columns
dim(counties_sf_utm_join)
```
##In above code When we converted EPA sites(tabular data frame) df to spatial data frame(this will be on hw)
...
###Number of 1974 births by county in NC
```
...
triCo <- counties_sf_utm %>% ###Choose relevant counties using filter function
```
...
myMap = ggplot() + 
  geom_sf(data = triCo)   ###triCo dataframe has 4 geometries or features in it
myMap
```
...
```{r}
triCo_centroids <-  st_centroid(triCo) ###this is a table with 4 rows with geometry column. Now we're overwriting geomeitres with values of centroids of the polygons
...
```{r}
triCo_centroids_2km <- st_buffer(triCo_centroids, 2000)  ###buffer centroids with 2000 meters distance
...
triCo_in2km <- st_buffer(triCo, -2000)  ###if you specify a negative number for your buffer, you can shrink a feature
...
triCo_centroids_chull <- triCo_centroids %>% 
  st_union() %>% ##take all centroids and 4 rows and combine them into one row
  st_convex_hull()   
...
triCo_centroids_voronoi <- triCo_centroids %>% 
  st_union() %>% ##union centroid point so they are one geometry 
  st_voronoi()  ##this is useful for interpolation. If you want to look at the areas between points (ex. distance around rain guages). This command then draws bounding boxes.
...
theSite_sfc <- st_sfc(theSite_sfp, crs = 4326) ##from point, he's creating a spatial feature column, which is the coordinates + the spatial reference
```


###Transform geographical coordinate system to a UTM  coordinate system
```{r}
...
###st_intersects says what feature in the counties dataset does this point intersect?
  ###The sparse command, instead of a full matrix of results, returns a column of trues and falses
```
...
selCounties <- counties_sf_utm[resultMask,]  ###using matrix subset, returns the countiess that are TRUE-->Durham
```
