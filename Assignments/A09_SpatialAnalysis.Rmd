...
##Convert back to sf dataframe-now will have only one geometry column.
```{r}
...
```{r, warning=FALSE, message=FALSE}
GagePlot<-ggplot() + 
  geom_sf(data = CountiesShapefile, col='black') + ###Counties
...
library(ggplot2)
GagePlot2<-ggplot() + 
 geom_sf(data=CountiesShapefile, color='indianred3') +
  geom_sf(data=NWISJoinCleansf, aes(size=gage_ht))+
...
labs(title='Stream Gage Locations and Magnitude in Nebraska Counties', color='County Name')+
guides(size=FALSE)  ###remove Legend for Size Aesthetic b/c it's not accurate and not showing magnitude differences
...
geom_sf(data=GageSumJoined, aes(fill=gage_ht))+
 scale_fill_gradient(name="Mean Stream Gage Height(meters)", low="white", high="tomato")+
  labs(title='Mean Stream Gage Heights by Nebraska Counties')
